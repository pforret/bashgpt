#!/usr/bin/env bash
### Created by Peter Forret ( pforret ) on 2023-05-15
...
readonly script_created="2023-05-15"
readonly run_as_root=-1 # run_as_root: 0 = don't check anything / 1 = script MUST run as root / -1 = script MAY NOT run as root
readonly script_description="An ai assistant in your CLI"
## some initialisation
action=""
script_prefix=""
script_basename=""
...
choice|1|action|action to perform|run,install,test,check,env,update
...
Script:main() {
  ...
  case "${action,,}" in
  run)
    #TIP: use «$script_prefix run» to start the conversation with ChatGPT
    #TIP:> $script_prefix run
    Os:require python3
    [[ -z "$OPENAI_API_KEY" ]] && IO:die "you need an OpenAI API key - get it from https://platform.openai.com/account/api-keys"

    IO:print "${txtInfo}------------------- START $script_basename -------------------${txtReset}"
    python3 "$script_install_folder/assistant.py"
    IO:print "${txtInfo}------------------- FINISH $script_basename -------------------${txtReset}"
   ;;

  install)
    #TIP: use «$script_prefix install» to install all required packages
    #TIP:> $script_prefix install

    Os:require pip python3

    # install https://pypi.org/project/python-dotenv/
    if ! pip show python-dotenv &> /dev/null ; then
      pip install python-dotenv
    fi
    IO:success "python-dotenv is installed"

    # install https://github.com/openai/openai-python
    # install https://pypi.org/project/revChatGPT/
    pip install --upgrade openai  &> /dev/null
    if ! pip show revChatGPT &> /dev/null ; then
      pip install revChatGPT
    fi
    IO:success "revChatGPT is installed"
  ...
  test)
    #TIP: use «$script_prefix models» to download the models
    python3 "$script_install_folder/test_api.py"
    ;;
